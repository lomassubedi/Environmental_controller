       ...
       : wxFrame(NULL, wxID_ANY, title,wxDefaultPosition,wxSize(390, 280), wxDEFAULT_FRAME_STYLE ^ wxRESIZE_BORDER),m_timer(this, ID_TIMER)
{
	...
	fileMenu->Append(Serial_Port, wxT("&Port Number\tAlt-P"), wxT("Select serial port"));

	fileMenu->Append(Serial_Baud, wxT("&Baud Rate\tAlt-P"), wxT("Enter baud rate"));
...
{
    wxMessageBox(wxString::Format(wxT("Serial Communication! \n \n")),
...
{
	if(com.Open()) txtRx->AppendText(wxString::Format(wxT("\nError opening port %d.\n"),com.GetPortNumber()));
	else txtRx->AppendText(wxString::Format(wxT("\nPort %d opened.\n"), com.GetPortNumber()));
}

void MyFrame::OnClose(wxCommandEvent& WXUNUSED(event))
{
	com.Close();
	txtRx->AppendText(wxString::Format(wxT("\nPort %d closed.\n"), com.GetPortNumber()));
}
...
void MyFrame::SelPort(wxCommandEvent& WXUNUSED(event))
{
	if (com.IsOpened()) {
		...
		long n = wxGetNumberFromUser(wxT("Enter the port number"), wxT("COM"), wxT("Set Port"), com.GetPortNumber(), 1, 99);
#else
		long n = wxGetNumberFromUser(wxT("Enter the port number (Add 100 to port number to use ttyUSB instead of ttyS)"), wxT("ttyUSB after adding 100 or ttyS"), wxT("Set Port"), com.GetPortNumber(), 0, 199);
#endif
		if (n > 0) {
			com.SetPortNumber(n);
			txtRx->AppendText(wxString::Format(wxT("\nPort %d selected.\n"), com.GetPortNumber()));
...
void MyFrame::SetBaud(wxCommandEvent& WXUNUSED(event))
{
	if (com.IsOpened()) {
		...
		long n = wxGetNumberFromUser(wxT("Enter the baud rate"), wxT("Baud rate"), wxT("Set Baud Rate"),com.GetBaudRate(),4800, 115200);
		if (n > 0) {
			com.SetBaudRate(n);
			txtRx->AppendText(wxString::Format(wxT("\nSet baud rate %d.\n"), com.GetBaudRate()));
		}
