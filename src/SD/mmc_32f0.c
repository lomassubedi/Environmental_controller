/*------------------------------------------------------------------------*/
/* STM32F030: MMCv3/SDv1/SDv2 (SPI mode) control module                   */
/*------------------------------------------------------------------------*/
/*
...
//    SysTick_Config(48000000 / 1000 - 1);
    ...
    GPIOA->MODER &= ~((3 << (SD_PIN_SCK << 1)) | (3 << (SD_PIN_MISO << 1)) |\
                    (3 << (SD_PIN_MOSI << 1)) | (3 << (SD_PIN_CS << 1)));
    GPIOA->MODER |= (2 << (SD_PIN_SCK << 1)) | (2 << (SD_PIN_MISO << 1)) |\
                    (2 << (SD_PIN_MOSI << 1)) | (2 << (SD_PIN_CS << 1));
    GPIOA->OSPEEDR |= (3 << (SD_PIN_SCK << 1)) | (3 << (SD_PIN_CS << 1)) |\
                    (3 << (SD_PIN_MOSI << 1));
    GPIOA->AFR[0] &= ~((0xf << (SD_PIN_SCK << 2)) |\
                    (0xf << (SD_PIN_MISO << 2)) |\
	...
	SPI1->CR1 = SPI_CR1_BR | SPI_CR1_MSTR | SPI_CR1_SSM | SPI_CR1_SPE;
...
)
{
    ...
    SPI1->CR2 = (SPI1->CR2 & ~SPI_CR2_DS) | 0;	/* Set SPI to 8-bit mode */
}
